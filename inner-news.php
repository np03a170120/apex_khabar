<?php include 'header.php'?>

<section class="inner-breadcrumb">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="#">बागमती प्रदेश</a></li>
            <li class="breadcrumb-item"><a href="#">राजनीति</a></li>
            <li class="breadcrumb-item active" aria-current="page">प्रदेश</li>
        </ol>
    </nav>
</section>

<!-- inner news -->
<section class="inner-news">
    <div class="container">
        <div class="row">
            <div class="col-md-9">
                <div class="big-news mb-0">
                    <div class="bn-title">
                        <h3>
                            भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने
                        </h3>
                    </div>
                    <div class="author-time">
                        <div class="author">
                            <div class="author-image">
                                <img src="./images/author-dummy.png" alt="">
                            </div>
                            <div class="author-title">
                                <h6>धिरेन्द्र शाक्य</h6>
                            </div>
                        </div>
                        <div class="time-stamp">
                            <div class="hmd-single">
                                <img src="./images/calender-icon-small.png" alt="">
                                <span> माघ १९, बुधबार</span>
                            </div>
                        </div>
                    </div>
                    <div class="bn-image">
                        <img src="./images/headling-news.png" alt="">
                    </div>
                </div>
                <div class="summary-news">
                    <div class="title-btn-wrapper">
                        <div class="sn-title">
                            <h6>सारङ्स पढ्नुहोस</h6>
                        </div>
                        <div class="accessibility-btns">
                            <button type="button" class="btn-small" onclick="fontDec()" title="Decrease Font Size">
                                अ-
                            </button>
                            <button type="button" class="btn-small" onclick="fontRes()" title="Reset Font Size">
                                अ
                            </button>
                            <button type="button" class="btn-small" onclick="fontInc()" title="Increase Font Size">
                                अ+
                            </button>
                            <button type="button" class="btn-small" title="Increase Font Size">
                                <img src="./images/news-only.png" alt="">
                            </button>
                        </div>
                    </div>
                    <ul class="sn-news">
                        <li><img src="./images/side-icon.png" alt="">एमसीसी पास नभए सत्ता गठबन्धन तोडिन्छ भन्ने
                            कुरासमेत भ्रममात्रै</li>
                        <li><img src="./images/side-icon.png" alt="">एमसीसी पास नभए सत्ता गठबन्धन तोडिन्छ भन्ने कुरासमेत
                            भ्रममात्रै</li>
                        <li><img src="./images/side-icon.png" alt="">एमसीसी पास नभए सत्ता गठबन्धन तोडिन्छ भन्ने कुरासमेत
                            भ्रममात्रै</li>
                    </ul>
                </div>

                <div class="bn-news" id="fontSize" style="font-size: 16px;">
                    <p>
                        उनले एमसीसी पास नभए सत्ता गठबन्धन तोडिन्छ भन्ने कुरासमेत भ्रममात्रै भएको दाबी गरेका छन् ।‘कुनै
                        भ्रममा नपरौं । प्रधानमन्त्री शेरबहादुर देउवाले एमसीसी सम्झौता पारित नगरे सत्तारुढ गठबन्धन
                        तोडिन्छ भनेर भन्नुभएको छैन, आजको बिन्दुसम्म उहाँले त्यसो भन्नुभएको छैन,’ नेता श्रेष्ठले सामाजिक
                        सञ्जालमा लेखेका छन्, ‘गठबन्धनमा एमसीसी कम्प्याक्ट सम्झौता पारित गर्ने सहमति भयो भन्ने कुरा पनि
                        सत्य होइन । यथास्थितिमा कमसेकम हाम्रो पार्टी त्यसको पक्षमा छैन ।’उनले एमसीसी पास नभए सत्ता
                        गठबन्धन तोडिन्छ भन्ने कुरासमेत भ्रममात्रै भएको दाबी गरेका छन् ।‘कुनै भ्रममा नपरौं ।
                        प्रधानमन्त्री शेरबहादुर देउवाले एमसीसी सम्झौता पारित नगरे सत्तारुढ गठबन्धन तोडिन्छ भनेर
                        भन्नुभएको छैन, आजको बिन्दुसम्म उहाँले त्यसो भन्नुभएको छैन,’ नेता श्रेष्ठले सामाजिक सञ्जालमा
                        लेखेका छन्, ‘गठबन्धनमा एमसीसी कम्प्याक्ट सम्झौता पारित गर्ने सहमति भयो भन्ने कुरा पनि सत्य होइन
                        । यथास्थितिमा कमसेकम हाम्रो पार्टी त्यसको पक्षमा छैन ।’
                    </p>
                    <!-- bigyas -->
                    <div class="inner-bigyas">
                        <div class="col-four-bigya ">
                            <a href="#">
                                <img src="./images/col-4-bigya.png" alt="">
                            </a>
                        </div>
                        <div class="col-four-bigya">
                            <a href="#">
                                <img src="./images/col-4-bigya.png" alt="">
                            </a>
                        </div>
                        <div class="col-four-bigya">
                            <a href="#">
                                <img src="./images/col-4-bigya.png" alt="">
                            </a>
                        </div>
                    </div>
                    <p>
                        उनले एमसीसी पास नभए सत्ता गठबन्धन तोडिन्छ भन्ने कुरासमेत भ्रममात्रै भएको दाबी गरेका छन् ।‘कुनै
                        भ्रममा नपरौं । प्रधानमन्त्री शेरबहादुर देउवाले एमसीसी सम्झौता पारित नगरे सत्तारुढ गठबन्धन
                        तोडिन्छ भनेर भन्नुभएको छैन, आजको बिन्दुसम्म उहाँले त्यसो भन्नुभएको छैन,’ नेता श्रेष्ठले सामाजिक
                        सञ्जालमा लेखेका छन्, ‘गठबन्धनमा एमसीसी कम्प्याक्ट सम्झौता पारित गर्ने सहमति भयो भन्ने कुरा पनि
                        सत्य होइन । यथास्थितिमा कमसेकम हाम्रो पार्टी त्यसको पक्षमा छैन ।’उनले एमसीसी पास नभए सत्ता
                        गठबन्धन तोडिन्छ भन्ने कुरासमेत भ्रममात्रै भएको दाबी गरेका छन् ।‘कुनै भ्रममा नपरौं ।
                        प्रधानमन्त्री शेरबहादुर देउवाले एमसीसी सम्झौता पारित नगरे सत्तारुढ गठबन्धन तोडिन्छ भनेर
                        भन्नुभएको छैन, आजको बिन्दुसम्म उहाँले त्यसो भन्नुभएको छैन,’ नेता श्रेष्ठले सामाजिक सञ्जालमा
                        लेखेका छन्, ‘गठबन्धनमा एमसीसी कम्प्याक्ट सम्झौता पारित गर्ने सहमति भयो भन्ने कुरा पनि सत्य होइन
                        । यथास्थितिमा कमसेकम हाम्रो पार्टी त्यसको पक्षमा छैन ।’
                    </p>
                </div>
                <div class="long-bigya">
                    <a href="#">
                        <img src="./images/long-bigya.png" alt="">
                    </a>
                </div>

                <!-- pratikriya -->
                <!-- <div class="fb-comments">
                    <div class="sec-title">
                        <h6>
                            प्रतिक्रिया
                        </h6>
                    </div>
                </div> -->
            </div>
            <div class="col-md-3">
                <div class="side-news">
                    <div class="sn-title">
                        <h6>सम्बन्धित खवर</h6>
                    </div>
                    <ul class="snn">
                        <li><a href="#">
                                <div class="snn-title">
                                    <span>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</span>
                                </div>
                            </a></li>
                        <li><a href="#">
                                <div class="snn-title">
                                    <span>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</span>
                                </div>
                            </a></li>
                        <li><a href="#">
                                <div class="snn-title">
                                    <span>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</span>
                                </div>
                            </a></li>
                        <li><a href="#">
                                <div class="snn-title">
                                    <span>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</span>
                                </div>
                            </a></li>
                    </ul>
                </div>
                <!-- bigyas -->
                <div class="col-four-bigya custom-side-bigya">
                    <a href="#">
                        <img src="./images/col-4-bigya.png" alt="">
                    </a>
                </div>
                <div class="col-four-bigya custom-side-bigya">
                    <a href="#">
                        <img src="./images/col-4-bigya.png" alt="">
                    </a>
                </div>
                <div class="col-four-bigya custom-side-bigya">
                    <a href="#">
                        <img src="./images/col-4-bigya.png" alt="">
                    </a>
                </div>
                <div class="col-four-bigya custom-side-bigya">
                    <a href="#">
                        <img src="./images/col-4-bigya.png" alt="">
                    </a>
                </div>
                <div class="col-four-bigya custom-side-bigya">
                    <a href="#">
                        <img src="./images/col-4-bigya.png" alt="">
                    </a>
                </div>
                <div class="col-four-bigya custom-side-bigya">
                    <a href="#">
                        <img src="./images/col-4-bigya.png" alt="">
                    </a>
                </div>
                <div class="col-four-bigya custom-side-bigya">
                    <a href="#">
                        <img src="./images/col-4-bigya.png" alt="">
                    </a>
                </div>
                <div class="col-four-bigya custom-side-bigya">
                    <a href="#">
                        <img src="./images/col-4-bigya.png" alt="">
                    </a>
                </div>
            </div>
        </div>
    </div>
</section>

<!-- recommended news -->
<section class="recom-news sec-padding">
    <div class="container">
        <div class="row">
            <div class="col-md-9">

                <div class="sec-title">
                    <h6>छुटाउनुभयो कि?</h6>
                </div>


                <div class="row">
                    <div class="col-md-6">
                        <div class="text-bigya-single">
                            <a href="#">
                                <div class="tbs-title font-size-16">
                                    <h6>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</h6>
                                </div>
                                <div class="tbs-desc">
                                    <p>पीएरे एमरिक आउबामियाङले आपसी समझदारीमा क्लब छाडेको आर्सनलले पुष्टि गरेको छ ।
                                        उनी
                                        तर,
                                        क्लबले
                                        गाबोनकातर, क्लबले गाबोनका आर्सनल छाडेर
                                        बार्सिलोना पुगेका छन् । तर, क्लबले गाबोनका</p>
                                </div>
                            </a>
                            <div class="md-author-date"><a href="#"></a><a href="#">
                                </a>
                                <div class="md-author"><a href="#">
                                    </a><a href="#">
                                        <img src="./images/author-icon.png" alt="">
                                        <span>चम्पा कुमारी</span>
                                    </a>
                                </div>
                                <div class="time-stamp ts-small">
                                    <div class="hmd-single">
                                        <img src="./images/calender-icon-small.png" alt="">
                                        <span> माघ १९, बुधबार</span>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="text-bigya-single">
                            <a href="#">
                                <div class="tbs-title font-size-16">
                                    <h6>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</h6>
                                </div>
                                <div class="tbs-desc">
                                    <p>पीएरे एमरिक आउबामियाङले आपसी समझदारीमा क्लब छाडेको आर्सनलले पुष्टि गरेको छ ।
                                        उनी
                                        तर,
                                        क्लबले
                                        गाबोनकातर, क्लबले गाबोनका आर्सनल छाडेर
                                        बार्सिलोना पुगेका छन् । तर, क्लबले गाबोनका</p>
                                </div>
                            </a>
                            <div class="md-author-date"><a href="#"></a><a href="#">
                                </a>
                                <div class="md-author"><a href="#">
                                    </a><a href="#">
                                        <img src="./images/author-icon.png" alt="">
                                        <span>चम्पा कुमारी</span>
                                    </a>
                                </div>
                                <div class="time-stamp ts-small">
                                    <div class="hmd-single">
                                        <img src="./images/calender-icon-small.png" alt="">
                                        <span> माघ १९, बुधबार</span>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="text-bigya-single">
                            <a href="#">
                                <div class="tbs-title font-size-16">
                                    <h6>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</h6>
                                </div>
                                <div class="tbs-desc">
                                    <p>पीएरे एमरिक आउबामियाङले आपसी समझदारीमा क्लब छाडेको आर्सनलले पुष्टि गरेको छ ।
                                        उनी
                                        तर,
                                        क्लबले
                                        गाबोनकातर, क्लबले गाबोनका आर्सनल छाडेर
                                        बार्सिलोना पुगेका छन् । तर, क्लबले गाबोनका</p>
                                </div>
                            </a>
                            <div class="md-author-date"><a href="#"></a><a href="#">
                                </a>
                                <div class="md-author"><a href="#">
                                    </a><a href="#">
                                        <img src="./images/author-icon.png" alt="">
                                        <span>चम्पा कुमारी</span>
                                    </a>
                                </div>
                                <div class="time-stamp ts-small">
                                    <div class="hmd-single">
                                        <img src="./images/calender-icon-small.png" alt="">
                                        <span> माघ १९, बुधबार</span>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="text-bigya-single">
                            <a href="#">
                                <div class="tbs-title font-size-16">
                                    <h6>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</h6>
                                </div>
                                <div class="tbs-desc">
                                    <p>पीएरे एमरिक आउबामियाङले आपसी समझदारीमा क्लब छाडेको आर्सनलले पुष्टि गरेको छ ।
                                        उनी
                                        तर,
                                        क्लबले
                                        गाबोनकातर, क्लबले गाबोनका आर्सनल छाडेर
                                        बार्सिलोना पुगेका छन् । तर, क्लबले गाबोनका</p>
                                </div>
                            </a>
                            <div class="md-author-date"><a href="#"></a><a href="#">
                                </a>
                                <div class="md-author"><a href="#">
                                    </a><a href="#">
                                        <img src="./images/author-icon.png" alt="">
                                        <span>चम्पा कुमारी</span>
                                    </a>
                                </div>
                                <div class="time-stamp ts-small">
                                    <div class="hmd-single">
                                        <img src="./images/calender-icon-small.png" alt="">
                                        <span> माघ १९, बुधबार</span>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="text-bigya-single">
                            <a href="#">
                                <div class="tbs-title font-size-16">
                                    <h6>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</h6>
                                </div>
                                <div class="tbs-desc">
                                    <p>पीएरे एमरिक आउबामियाङले आपसी समझदारीमा क्लब छाडेको आर्सनलले पुष्टि गरेको छ ।
                                        उनी
                                        तर,
                                        क्लबले
                                        गाबोनकातर, क्लबले गाबोनका आर्सनल छाडेर
                                        बार्सिलोना पुगेका छन् । तर, क्लबले गाबोनका</p>
                                </div>
                            </a>
                            <div class="md-author-date"><a href="#"></a><a href="#">
                                </a>
                                <div class="md-author"><a href="#">
                                    </a><a href="#">
                                        <img src="./images/author-icon.png" alt="">
                                        <span>चम्पा कुमारी</span>
                                    </a>
                                </div>
                                <div class="time-stamp ts-small">
                                    <div class="hmd-single">
                                        <img src="./images/calender-icon-small.png" alt="">
                                        <span> माघ १९, बुधबार</span>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="text-bigya-single">
                            <a href="#">
                                <div class="tbs-title font-size-16">
                                    <h6>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</h6>
                                </div>
                                <div class="tbs-desc">
                                    <p>पीएरे एमरिक आउबामियाङले आपसी समझदारीमा क्लब छाडेको आर्सनलले पुष्टि गरेको छ ।
                                        उनी
                                        तर,
                                        क्लबले
                                        गाबोनकातर, क्लबले गाबोनका आर्सनल छाडेर
                                        बार्सिलोना पुगेका छन् । तर, क्लबले गाबोनका</p>
                                </div>
                            </a>
                            <div class="md-author-date"><a href="#"></a><a href="#">
                                </a>
                                <div class="md-author"><a href="#">
                                    </a><a href="#">
                                        <img src="./images/author-icon.png" alt="">
                                        <span>चम्पा कुमारी</span>
                                    </a>
                                </div>
                                <div class="time-stamp ts-small">
                                    <div class="hmd-single">
                                        <img src="./images/calender-icon-small.png" alt="">
                                        <span> माघ १९, बुधबार</span>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="text-bigya-single">
                            <a href="#">
                                <div class="tbs-title font-size-16">
                                    <h6>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</h6>
                                </div>
                                <div class="tbs-desc">
                                    <p>पीएरे एमरिक आउबामियाङले आपसी समझदारीमा क्लब छाडेको आर्सनलले पुष्टि गरेको छ ।
                                        उनी
                                        तर,
                                        क्लबले
                                        गाबोनकातर, क्लबले गाबोनका आर्सनल छाडेर
                                        बार्सिलोना पुगेका छन् । तर, क्लबले गाबोनका</p>
                                </div>
                            </a>
                            <div class="md-author-date"><a href="#"></a><a href="#">
                                </a>
                                <div class="md-author"><a href="#">
                                    </a><a href="#">
                                        <img src="./images/author-icon.png" alt="">
                                        <span>चम्पा कुमारी</span>
                                    </a>
                                </div>
                                <div class="time-stamp ts-small">
                                    <div class="hmd-single">
                                        <img src="./images/calender-icon-small.png" alt="">
                                        <span> माघ १९, बुधबार</span>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="text-bigya-single">
                            <a href="#">
                                <div class="tbs-title font-size-16">
                                    <h6>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</h6>
                                </div>
                                <div class="tbs-desc">
                                    <p>पीएरे एमरिक आउबामियाङले आपसी समझदारीमा क्लब छाडेको आर्सनलले पुष्टि गरेको छ ।
                                        उनी
                                        तर,
                                        क्लबले
                                        गाबोनकातर, क्लबले गाबोनका आर्सनल छाडेर
                                        बार्सिलोना पुगेका छन् । तर, क्लबले गाबोनका</p>
                                </div>
                            </a>
                            <div class="md-author-date"><a href="#"></a><a href="#">
                                </a>
                                <div class="md-author"><a href="#">
                                    </a><a href="#">
                                        <img src="./images/author-icon.png" alt="">
                                        <span>चम्पा कुमारी</span>
                                    </a>
                                </div>
                                <div class="time-stamp ts-small">
                                    <div class="hmd-single">
                                        <img src="./images/calender-icon-small.png" alt="">
                                        <span> माघ १९, बुधबार</span>
                                    </div>
                                </div>
                            </div>

                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="side-news">
                    <div class="sn-title">
                        <h6>ट्रेन्डिङ</h6>
                    </div>
                    <ul class="snn">
                        <li><a href="#">
                                <div class="snn-title">
                                    <span>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</span>
                                </div>
                            </a></li>
                        <li><a href="#">
                                <div class="snn-title">
                                    <span>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</span>
                                </div>
                            </a></li>
                        <li><a href="#">
                                <div class="snn-title">
                                    <span>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</span>
                                </div>
                            </a></li>
                        <li><a href="#">
                                <div class="snn-title">
                                    <span>भोलिदेखि आइतबारसम्म असिनासहित वर्षा हुनसक्ने</span>
                                </div>
                            </a></li>
                    </ul>
                </div>
                <!-- bigyas -->
                <div class="col-four-bigya custom-side-bigya">
                    <a href="#">
                        <img src="./images/col-4-bigya.png" alt="">
                    </a>
                </div>
                <div class="col-four-bigya custom-side-bigya">
                    <a href="#">
                        <img src="./images/col-4-bigya.png" alt="">
                    </a>
                </div>
                <div class="col-four-bigya custom-side-bigya">
                    <a href="#">
                        <img src="./images/col-4-bigya.png" alt="">
                    </a>
                </div>
            </div>
        </div>
    </div>
</section>

<?php include 'footer.php'?>